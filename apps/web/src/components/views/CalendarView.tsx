    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-white">
      ...
      <div className="grid grid-cols-7 border-b border-gray-100">
        {DAYS.map((d) => (
          <div
            key={d}
            className="text-center text-[11px] font-medium text-gray-400 py-2.5"
          >
            {d}
          </div>
        ))}
      ...
      {isLoading ? (
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-7 grid-rows-6">
          {calendarDays.map(({ date, tasks: dayTasks }, idx) => {
            if (!date) {
              return <div key={`pad-${idx}`} className="border-r border-b border-gray-100 min-h-28" />;
            }
            const isToday = date.toISOString().split('T')[0] === todayStr;
            return (
              <div
                key={date.toISOString()}
                className={`border-r border-b border-gray-100 min-h-28 p-1.5 ${
                  isToday ? 'bg-blue-50/30' : ''
                }`}
              >
                <span
                  className={`text-xs font-medium ${
                    isToday ? 'text-blue-600 bg-blue-100 rounded-full w-5 h-5 flex items-center justify-center' : 'text-gray-500'
                  }`}
                >
                  {date.getDate()}
                </span>
                <div className="mt-0.5 space-y-0.5">
                  {dayTasks.slice(0, 3).map((task) => (
                    <button
                      ...
                      className={`w-full text-left text-[11px] truncate px-1.5 py-0.5 rounded ${STATUS_COLORS[task.status] ?? ''}`}
                    >
                      {task.title}
                    </button>
                  ))}
