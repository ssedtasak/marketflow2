...
function Column({ column, tasks, onTaskClick, }) {
    return (_jsxs("div", { className: "flex flex-col w-72 flex-shrink-0 bg-gray-50/50 rounded-2xl", children: [_jsxs("div", { className: "px-4 py-3.5 flex items-center justify-between", children: [_jsx("span", { className: "text-xs font-semibold text-gray-600 uppercase tracking-wide", children: column.label }), _jsx("span", { className: "text-xs text-gray-400 bg-white px-2 py-0.5 rounded-full border border-gray-200/60 font-medium shadow-sm", children: tasks.length })] }), _jsxs("div", { className: "flex-1 overflow-y-auto px-3 pb-3", children: [_jsx(SortableContext, { items: tasks.map((t) => t.id), strategy: verticalListSortingStrategy, children: tasks.map((task) => (_jsx(TaskCard, { task: task, onClick: () => onTaskClick(task) }, task.id))) }), tasks.length === 0 && (_jsx("div", { className: "border-2 border-dashed border-gray-200/80 rounded-xl p-6 text-center", children: _jsx("p", { className: "text-xs text-gray-400", children: "Drop tasks here" }) }))] })] }));
}
